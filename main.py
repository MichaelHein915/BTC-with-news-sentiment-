...
def train_and_evaluate(df_daily):
    ...
    # -- Step 4: Train LightGBM + XGBoost (Model A) -------------------------
    ...
    # -- Steps 5-6-7 for both models ---------------------------------------
    for model_key, label in [("lgbm", "LightGBM"), ("xgb", "XGBoost")]:
        model_a = model_a_results[model_key]["model"]
        pipeline = run_pipeline_for_model(
            label, model_a, df_merged, tech_features, sent_features, mask
        )

        # Save the best model as the bundle
        if model_key == model_a_results["best"]:
            bundle = {
            ...
            print(f"\nBest model ({label}) saved -> {MODEL_BUNDLE_PATH}")
