...
def main():
    print("=" * 70)
    print("BTC/USD Model Comparison (Ablations + Two-stage, LightGBM + XGBoost)")
    print("=" * 70)
    ...
    for variant_name, features in feat_sets.items():
        ...
        for model_key, model_label in [("lgbm", "LightGBM"), ("xgb", "XGBoost")]:
            model_a = model_a_results[model_key]["model"]
            ...
            eval_results = evaluate_model_a_only(
                ...
                features=features,
                mask=mask,
                model_label=f"{variant_name}_{model_label}",
            )
            bt_results = run_backtest(eval_results, model_label=f"{variant_name}_{model_label}")
            ...
            rows.append({
                ...
                "backtest_return_pct": bt_results[0]["total_return"],
                "backtest_sharpe": bt_results[0]["sharpe"],
                "backtest_max_dd": bt_results[0]["max_drawdown"],
            })

    # Two-stage evaluation for both models
    ...
    for model_key, model_label in [("lgbm", "LightGBM"), ("xgb", "XGBoost")]:
        print("\n" + "-" * 70)
        print(f"VARIANT: Two-stage ({model_label} Model A + residual Model B)")
        print("-" * 70)
        ...
        eval_results = evaluate_ensemble(
            ...
            mask=mask,
            model_label=f"TwoStage_{model_label}",
        )
        bt_results = run_backtest(eval_results, model_label=f"TwoStage_{model_label}")
        ...
        rows.append({
            ...
            "backtest_return_pct": bt_results[1]["total_return"],
